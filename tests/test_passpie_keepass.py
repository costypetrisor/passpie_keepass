...
from passpie_keepass import passpie_keepass


def test_kppy_matching_returns_false_when_kppy_is_not_installed(mocker):
    mocker.patch.object(passpie_keepass, '_found_kppy', False)
    importer = passpie_keepass.KppyImporter()
    importer.log = Mock()
...
def test_kppy_matching_returns_false_when_path_to_match_is_not_a_file(mocker):
    mocker.patch.object(passpie_keepass, '_found_kppy', True)
    importer = passpie_keepass.KppyImporter()
    importer.log = Mock()
...
class TestWithAKeepassDb(object):
    ...
    def test_works_to_import(self, mocker, kppy_db):
        mocker.patch.object(passpie_keepass.click, 'prompt',
    ...
    def test_returns_empty_collection_if_passed_a_corrupt_file(self, mocker, tmpdir):
        mocker.patch.object(
            passpie_keepass.click, 'prompt', return_value='fake_pass')
